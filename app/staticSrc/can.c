...
void initCanPck(canTxStruct_t *canPkt, uint32_t id, bool ext, uint8_t data[], uint8_t length) {
  canPkt->canport=CAN1;
  canPkt->id = id;
  canPkt->ext = ext;
  canPkt->rtr = false;
  canPkt->fmi = 0;
  canPkt->length = length;
  for(uint8_t i=0;i<length;i++) {
  canPkt->data[i] = data[i];
  }
  canPkt->timestamp =0;
}

void canReceive(canRxStruct_t *canPkt, bool release) {
	canPkt->canport=CAN1;
	uint32_t fifo_id = 0;
	bool fifo = 0;
	union {
	...
	/* Get type of CAN ID and CAN ID. */
	if (CAN_RIxR(canPkt->canport , fifo_id) & CAN_RIxR_IDE) {
		canPkt->ext = true;
		/* Get extended CAN ID. */
		canPkt->id = (CAN_RIxR(canPkt->canport, fifo_id) >> CAN_RIxR_EXID_SHIFT) &
			CAN_RIxR_EXID_MASK;
	} else {
		canPkt->ext = false;
		/* Get standard CAN ID. */
		canPkt->id = (CAN_RIxR(canPkt->canport, fifo_id) >> CAN_RIxR_STID_SHIFT) & CAN_RIxR_STID_MASK;
	}

	/* Get remote transmit flag. */
	if (CAN_RIxR(canPkt->canport, fifo_id) & CAN_RIxR_RTR) {
		canPkt->rtr = true;
	} else {
		canPkt->rtr = false;
	}
	...
	/* Get filter match ID. */
	canPkt->fmi = ((CAN_RDTxR(canPkt->canport, fifo_id) & CAN_RDTxR_FMI_MASK) >>
		CAN_RDTxR_FMI_SHIFT);

	/* Get data length. */
	canPkt->length = CAN_RDTxR(canPkt->canport, fifo_id) & CAN_RDTxR_DLC_MASK;
	/* accelerate reception by copying the CAN data from the controller
	 * memory to the fast internal RAM
	 */

	if (canPkt->timestamp) {
		canPkt->timestamp = (CAN_RDTxR(canPkt->canport, fifo_id) &
			CAN_RDTxR_TIME_MASK) >> CAN_RDTxR_TIME_SHIFT;
	}

	rdlxr.data32 = CAN_RDLxR(canPkt->canport, fifo_id);
	rdhxr.data32 = CAN_RDHxR(canPkt->canport, fifo_id);
	/* */
	/* Get data.
	 * Byte wise copy is needed because we do not know the alignment
	 * of the input buffer.
	 ...
	 * to the function, but it is not the case
	 */
	canPkt->data[0] = rdlxr.data8[0];
	canPkt->data[1] = rdlxr.data8[1];
	canPkt->data[2] = rdlxr.data8[2];
	canPkt->data[3] = rdlxr.data8[3];
	canPkt->data[4] = rdhxr.data8[0];
	canPkt->data[5] = rdhxr.data8[1];
	canPkt->data[6] = rdhxr.data8[2];
	canPkt->data[7] = rdhxr.data8[3];

	/* Release the FIFO. */
	if(release) {
		if (fifo) {
		CAN_RF1R(canPkt->canport) |= CAN_RF1R_RFOM1;
		} else {
		CAN_RF0R(canPkt->canport) |= CAN_RF1R_RFOM1;
		}
	}
}

int canTransmit(canTxStruct_t *canPkt) {
	int ret = 0;
	uint32_t mailbox = 0;
	union {
		uint8_t data8[4];
	...
	/* Check which transmit mailbox is empty if any. */
	if ((CAN_TSR(canPkt->canport) & CAN_TSR_TME0) == CAN_TSR_TME0) {
		ret = 0;
		mailbox = CAN_MBOX0;
	} else if ((CAN_TSR(canPkt->canport) & CAN_TSR_TME1) == CAN_TSR_TME1) {
		ret = 1;
		mailbox = CAN_MBOX1;
	} else if ((CAN_TSR(canPkt->canport) & CAN_TSR_TME2) == CAN_TSR_TME2) {
		ret = 2;
		mailbox = CAN_MBOX2;
	} else {
		ret = -1;
	...
	if (canPkt->ext) {
		/* Set extended ID. */
		CAN_TIxR(canPkt->canport, mailbox) = (canPkt->id << CAN_TIxR_EXID_SHIFT) |
			CAN_TIxR_IDE;
	} else {
		/* Set standard ID. */
		CAN_TIxR(canPkt->canport, mailbox) = canPkt->id << CAN_TIxR_STID_SHIFT;
	}

	/* Set/clear remote transmission request bit. */
	if (canPkt->rtr) {
		CAN_TIxR(canPkt->canport, mailbox) |= CAN_TIxR_RTR; /* Set */
	}

	/* Set the DLC. */
	CAN_TDTxR(canPkt->canport, mailbox) &= ~CAN_TDTxR_DLC_MASK;
	CAN_TDTxR(canPkt->canport, mailbox) |= (canPkt->length & CAN_TDTxR_DLC_MASK);

	switch (canPkt->length) {
	case 8:
		tdhxr.data8[3] = canPkt->data[7];
		/* fall through */
	case 7:
		tdhxr.data8[2] = canPkt->data[6];
		/* fall through */
	case 6:
		tdhxr.data8[1] = canPkt->data[5];
		/* fall through */
	case 5:
		tdhxr.data8[0] = canPkt->data[4];
		/* fall through */
	case 4:
		tdlxr.data8[3] = canPkt->data[3];
		/* fall through */
	case 3:
		tdlxr.data8[2] = canPkt->data[2];
		/* fall through */
	case 2:
		tdlxr.data8[1] = canPkt->data[1];
		/* fall through */
	case 1:
		tdlxr.data8[0] = canPkt->data[0];
		/* fall through */
	default:
		break;
	}
	...
	CAN_TDLxR(canPkt->canport, mailbox) = tdlxr.data32;
	CAN_TDHxR(canPkt->canport, mailbox) = tdhxr.data32;

	/* Request transmission. */
	CAN_TIxR(canPkt->canport, mailbox) |= CAN_TIxR_TXRQ;

	if(CAN_TSR(canPkt->canport) & (CAN_TSR_TERR0|CAN_TSR_TERR1|CAN_TSR_TERR2)) {
		canErrorHandler(transmitError);
	}
	return ret;
}

void canErrorHandler(ErrorCode_t error) {
	if(error == receiveError) {
	/* Receive Error detected */
	} else if (error == transmitError) {
